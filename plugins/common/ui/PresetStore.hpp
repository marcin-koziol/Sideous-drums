/*
 * Sideous Drums - simple file-based preset storage, entirely UI-side (no DPF
 ...
 * array). One plain-text ".sdpreset" file per preset, one "symbol=value"
 ...
 * same failure across every sideous-* plugin that copied this file, fixed
 * first in sideous-noise). POSIX mkdir/opendir/readdir/remove carry no such
 * version gating.
 */
...
inline std::string presetsDirectory()
{
    ...
    return appData ? std::string(appData) + "\\sideous-drums\\presets" : ".\\presets";
#elif defined(__APPLE__)
    const char* home = std::getenv("HOME");
    return home ? std::string(home) + "/Library/Application Support/sideous-drums/presets" : "./presets";
#else
    if (const char* xdg = std::getenv("XDG_DATA_HOME"))
        return std::string(xdg) + "/sideous-drums/presets";
    const char* home = std::getenv("HOME");
    return home ? std::string(home) + "/.local/share/sideous-drums/presets" : "./presets";
#endif
}
...
// sorted (alphabetical) list of preset display names - the ".sdpreset"
...
inline std::vector<std::string> listPresets()
{
    ...
    static constexpr const char* kExt = ".sdpreset";
...
inline bool savePreset(const std::string& name, const float* values)
{
    ...
    std::ofstream out(presetsDirectory() + "/" + name + ".sdpreset");
    if (!out.is_open())
        return false;
...
{
    std::ifstream in(presetsDirectory() + "/" + name + ".sdpreset");
    if (!in.is_open())
        return false;
...
{
    return std::remove((presetsDirectory() + "/" + name + ".sdpreset").c_str()) == 0;
}
